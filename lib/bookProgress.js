...
export default async function BookProgress(user, googleBooksId) {
  try {
    ...
    const bookIndex = userActiveBooks.find((activeBook)=>{
    return activeBook.$id ==activeBookDocument.$id
    });
    console.log("bookIndex: ",bookIndex)
    let bookId; // Gemeinsame Variable zur Speicherung der Buch-ID

    if (activeBookDocument) {
      bookId = activeBookDocument.$id;
      if (!bookIndex) {
        userActiveBooks.push(activeBookDocument.$id);
